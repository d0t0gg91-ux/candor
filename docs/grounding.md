...
Candor's ten work-mode personas are not invented dispositions. Each one is a trait
...
4. **The Predictive Index four-drive model** (Dominance, Extraversion, Patience,
   ...
   and Analyzer). A work-behavior lens that maps cleanly onto the same traits.
...
- **High Conscientiousness** -> closure and follow-through; good for shipping and for
  decisions, a liability for open-ended exploration.
...
one; _Security_, _Writing_, _Decision_, and _Data_ are anchored instead on their 16-type
role family and Predictive-Index profile (Guardian, Persuader, Captain, Analyzer), which
fit those dispositions more precisely than forcing a single archetype would. Each mode's
`SKILL.md` translates its trait signature into concrete behavioral directives and a
before/after example.

## The shared core, and why it's calibrated

Underneath all ten sits `candor-core`. Its job is to counter the specific ways an
