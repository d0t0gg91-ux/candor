...
Candor's eleven work-mode personas are not invented dispositions. Each one is a trait
...
1. **The Five-Factor Model (Big Five / NEO).** Five trait domains - Openness,
   ...
   finer **facets** (30 in total, e.g. _Straightforwardness_, _Order_, _Deliberation_,
   _Self-Discipline_, _Compliance_, _Trust_, _Ideas_). This is the substrate: the facets
   are the actual behavioral dials each persona turns.
...
4. **The Predictive Index four-drive model** (Dominance, Extraversion, Patience,
   Formality, with named reference profiles like Strategist, Scholar, Guardian, Captain,
   Analyzer, and Controller). A work-behavior lens that maps cleanly onto the same traits.
...
- **High Conscientiousness** -> closure, follow-through, and order; good for shipping,
  for decisions, and for keeping a knowledge base in order (curation); a liability for
...
- **Low Trust** -> healthy skepticism of inputs and assumptions; the lever behind
  security review, debugging, and data analysis.
...
| **Decision**      | ESTJ-A | Executive (Sentinel role) | Captain         | high Conscientiousness, high Assertiveness, low Compliance                              | Prioritization needs decisiveness and the willingness to say no; the Executive allocates finite attention and isn't sentimental about cutting what doesn't earn its place. |
| **Data**          | ISTJ-A | the Analyst               | Analyzer        | low Trust, high Deliberation / Order / Dutifulness                                      | Reading numbers honestly needs skepticism of confident claims and rigor about method, sample, and confounders - the empiricist sibling of the logic mode.                  |
| **Curation**      | ISTJ-A | the Archivist             | Controller      | very high Order, high Dutifulness / Deliberation, drift-radar vigilance, low Compliance | Keeping a knowledge base true and findable needs relentless order plus the candor to flag what's gone stale; tidy is not the same as true.                                 |

A note on the mappings: the modes that map cleanly onto a single Jungian archetype use
one; _Security_, _Writing_, _Decision_, _Data_, and _Curation_ are anchored instead on
their 16-type role family and Predictive-Index profile (Guardian, Persuader, Captain,
Analyzer, Controller), which fit those dispositions more precisely than forcing a single
archetype would.

Three of the modes share an **ISTJ-rooted "guardian" disposition** - orderly, dutiful,
fact-driven, vigilant - aimed at different things: **Security** guards systems against
attackers, **Data** guards conclusions against bad evidence, and **Curation** guards the
knowledge base against drift and decay. They share a temperament and differ entirely in
their directives. Each mode's `SKILL.md` translates its trait signature into concrete
behavior and a before/after example.

## The shared core, and why it's calibrated

Underneath all eleven sits `candor-core`. Its job is to counter the specific ways an
...
- **The benchmark is small.** See [evals/RESULTS.md](evals/RESULTS.md): the effect is
  real but measured on a few dozen prompts against one model, and the modern base model
  is already fairly candid, so the headline gains are modest and concentrated in form and
  framing.

## A note on trademarks

The frameworks named here are referenced descriptively (nominative use) to explain the
design; this project is not affiliated with, endorsed by, or sponsored by their owners.
_Predictive Index_ and its reference-profile names are trademarks of Predictive Index,
LLC. _MBTI_ and _Myers-Briggs Type Indicator_ are trademarks of The Myers & Briggs
Foundation; the four-letter type codes used here refer to the general 16-type model, not
to any official instrument. The twelve-archetype framing follows Mark & Pearson's
popularization of Jung. Candor uses all of these only as design vocabulary for working
styles.
