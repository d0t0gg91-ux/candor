...
![Status: experimental](<https://img.shields.io/badge/status-experimental%20(v0.5.0)-orange.svg>)
...
rationale and the trait signatures behind each mode. Each persona also ships as a subagent (the `/agents` picker lists them as
`candor:candor-<mode>`), and [AGENTS.md](AGENTS.md) has the same prompts as standalone,
copy-pasteable versions.
